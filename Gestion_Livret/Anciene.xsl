<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/"> 
		...
		<xsl:text>'),'</xsl:text>
		<xsl:value-of select="$filiere"/>
		<xsl:text>',(SELECT _ID_COMPO_ FROM livret_compo WHERE _LIBELLE_COMPO_ = ' </xsl:text>
		<xsl:value-of select="$compo"/>
		<xsl:text>')</xsl:text>
		<xsl:text>);&#xA;</xsl:text>
		...
		<xsl:text>INSERT IGNORE INTO livret_parcours (_ID_FILIERE_, _ID_PROMO_) VALUES ((SELECT _ID_COMPO_ FROM livret_filiere WHERE _LIBELLE_FILIERE_ = ' </xsl:text>
		<xsl:value-of select="$filiere"/>
		<xsl:text>'),(SELECT _ID_COMPO_ FROM livret_promotion WHERE _LIBELLE_PROMO_ = ' </xsl:text>
	...
	<xsl:template match="module">
		...
		<xsl:text>INSERT IGNORE INTO livret_matiere (_ID_MAT_, _ID_PROMO_, _LIBELLE_MAT_, _CODE_APOG_MAT_, _ID_UE_) VALUES ((SELECT _ID_MAT_ FROM livret_matiere WHERE _ID_PROMO_=(SELECT _ID_COMPO_ FROM livret_promotion WHERE _LIBELLE_PROMO_ = ' </xsl:text>
		<xsl:value-of select="$promo"/>
		<xsl:text>') AND _LIBELLE_PROMO_='</xsl:text>
		...
		<xsl:text>INSERT IGNORE INTO livret_semestre (_ID_PROMO_, _ID_MAT_, _SEMESTRE_) VALUES ((SELECT _ID_COMPO_ FROM livret_promotion WHERE _LIBELLE_PROMO_ = ' </xsl:text>
		<xsl:value-of select="$promo"/>
		<xsl:text>'), (SELECT _ID_MAT_ FROM livret_matiere WHERE _ID_PROMO_ = (SELECT _ID_COMPO_ FROM livret_promotion WHERE _LIBELLE_PROMO_ = ' </xsl:text>
		<xsl:value-of select="$promo"/>
		<xsl:text>') AND _LIBELLE_PROMO_='</xsl:text>
		<xsl:value-of select="./titre"/>
		<xsl:text>'), '</xsl:text>
		...
		<xsl:text>INSERT IGNORE INTO livret_module (_ID_MAT_ , _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _NB_ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_,_MOD_CC_1_,_MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES ((SELECT _ID_MAT_ FROM livret_matiere WHERE _ID_PROMO_ = (SELECT _ID_COMPO_ FROM livret_promotion WHERE _LIBELLE_PROMO_ = ' </xsl:text>
		<xsl:value-of select="$promo"/>
		<xsl:text>') AND _LIBELLE_PROMO_='</xsl:text>
		<xsl:value-of select="./titre"/>
		<xsl:text>'), '</xsl:text>
		...
		<xsl:for-each select="./lesPrerequis/prerequis">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:text> INSERT IGNORE INTO livret_pre_requis (_ID_MAT_FILS_, _ID_MAT_,_DESCRIPTION_) VALUES ((SELECT _ID_MAT_ FROM livret_matiere WHERE _ID_PROMO_ = (SELECT _ID_COMPO_ FROM livret_promotion WHERE _LIBELLE_PROMO_ = ' </xsl:text>
					<xsl:value-of select="$promo"/>
					<xsl:text>') AND _LIBELLE_PROMO_='</xsl:text>
					<xsl:value-of select="./titre"/>
					<xsl:text>'), </xsl:text>
		...
		<xsl:if test="./nomPremierResp">
			...
			<xsl:text>INSERT IGNORE INTO livret_responsable_module (_ID_MAT_, _ID_ENS_) VALUES ((SELECT _ID_MAT_ FROM livret_matiere WHERE _ID_PROMO_ = (SELECT _ID_COMPO_ FROM livret_promotion WHERE _LIBELLE_PROMO_ = ' </xsl:text>
			<xsl:value-of select="$promo"/>
			<xsl:text>') AND _LIBELLE_PROMO_='</xsl:text>
			<xsl:value-of select="./titre"/>
			<xsl:text>'), (SELECT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_=' </xsl:text>
			<xsl:value-of select="$lastName"/>
			<xsl:text>' AND _PRENOM_ENS_='</xsl:text>
			<xsl:value-of select="$firstName"/>
			<xsl:text>' AND _EMAIL_ENS_='</xsl:text>
