...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'PY', 'Interférences et diffraction', '4', '0', '0', '18', '30', '4', '4', 'Français_', 'Donner les connaissances de base en optique ondulatoire.', '*
...
*
Diffraction : - Principe d\'Huygens-Fresnel, diffraction par une fente fine, 2 fentes fines; - Diffraction par $N$ fentes fines, réseau en transmission ; - Diffraction par une ouverture circulaire, résolution des instruments d\'optique.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Avoir suivi le module d\'Optique au semestre 1.', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Interférences et diffraction' AND _ECTS_='4' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Norbert' AND _PRENOM_ENS_='Garnier' AND _EMAIL_ENS_='noureddine.el\_jaouhari@univ-orleans.fr');
...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'PY', 'Lois fondamentales d\'électricité linéaire', '1', '0', '0', '12', '24', '3', '3', 'Français_', 'Acquérir les notions de base en électricité. Savoir analyser et câbler un circuit constitué de dipôles linéaires. Mesurer des tensions, des fréquences et des déphasages.', '\'Etude des circuits électriques linéaires en continu et alternatif. Les notions abordées sont les dipôles passifs et actifs, leurs associations, les théorèmes de Thévenin et de Norton, la puissance instantanée et la puissance complexe, les fonctions de transfert, la réponse temporelle d\'un circuit et le régime transitoire. Les éléments de mathématiques (nombres complexes, équations aux dérivées ordinaires) seront utilisées dans ce module.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', '', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Lois fondamentales d\'électricité linéaire' AND _ECTS_='3' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Pascal' AND _PRENOM_ENS_='Andreazza' AND _EMAIL_ENS_='alexandre.tessier@univ-orleans.fr');
...
Relativité :: noyaux et radioactivité; accélérateurs et collisionneurs ; forces faible et forte; familles des particules ; le photon comme boson; effets Compton.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Mécanique du point; Champs électrostatiques; Magnétisme et ondes dans le vide; Mathématiques pour sciences physiques 1,2 et 3; physique quantique pour l\'option en S6.', 'Ressources', 'Biblio', '', 'Unité optionnelle : au choix parmi 3.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Relativité et physique subatomique' AND _ECTS_='' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Alessandro' AND _PRENOM_ENS_='Spallicci Di Filottrano' AND _EMAIL_ENS_='pierre.debs@univ-orleans.fr');
...
* Systèmes discrets à $n$ degrés de liberté(avec n grand), propagation d\'une onde. Introduction aux systèmes continus : vibrations transversales des cordes,longitudinales des barres, libres et exitées.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Mécanique du point (principaux théorèmes de conservation). Mécanique du solide (moment d\'inertie, théorèmes de Huygens et Koenig.', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Mécanique analytique' AND _ECTS_='3' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Isabelle' AND _PRENOM_ENS_='Rannou' AND _EMAIL_ENS_='nawfal.elhage\_hassan@univ-orleans.fr');
...
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Fonctions de plusieurs variables, courbes et surfaces paramétrées' AND _ECTS_='5' AND _CODE_MAT_='MA');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Martin' AND _PRENOM_ENS_='Grensing' AND _EMAIL_ENS_='noureddine.el\_jaouhari@univ-orleans.fr');
...
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Mécanique des solides et vibration' AND _ECTS_='6' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Francesco' AND _PRENOM_ENS_='Piazza' AND _EMAIL_ENS_='vittoria.pierfelice@univ-orleans.fr');
...
* Ondes électromagnétiques progressives planes monochromatiques dans le vide;structure de l\'onde; superposition d\'ondes planes, ondes stationnaires; polarisation de l\'onde; transport de l\'énergie, vecteur de Poynting, intensité des ondes électromagnétiques.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Champs électrostatiques; Mécanique du point; Mathématiques pour les sciences physiques 1 & 2', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Magnétisme dans le vide' AND _ECTS_='4' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Orélien' AND _PRENOM_ENS_='Randriamboarison' AND _EMAIL_ENS_='noureddine.el\_jaouhari@univ-orleans.fr');
...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'PY', 'Physique quantique', '6', '18', '18', '0', '0', '4', '4', 'Français_', 'Introduction aux concepts de la physique quantique.', '* Concepts et phénomènes quantiques. Dualité onde-corpuscule. Inégalités de Heisenberg. 
...
* Potentiels périodiques. Notions de bandes d\'énergie. Expérience de Sterne et Guerlach.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Mathématiques pour sciences physiques 1, 2 et 3.', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Physique quantique' AND _ECTS_='4' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Caroline' AND _PRENOM_ENS_='Andreazza' AND _EMAIL_ENS_='pierre.debs@univ-orleans.fr');
...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'MA', 'Analyse numérique matricielle', '5', '0', '0', '0', '24', '3', '3', 'Français_', '', '* Inversion de matrices,
  ...
  * Moindres carrés,
  * Mise en \oe uvre avec le logiciel Scilab de calcul matriciel.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Avoir suivi ``Mathématiques numériques\'\' au semestre 3 et l\'ensemble des modules d\'algèbre des semestres précédents.', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Analyse numérique matricielle' AND _ECTS_='3' AND _CODE_MAT_='MA');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
...
* Forces centrales, forces de gravitation, satellites.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Bases de la mécanique enseignées dans le secondaire : UE - mathématiques pour sciences physiques 1.', 'Ressources', 'Biblio', '', 'Unité optionnelle. Option 1 (parmi 3).');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Mécanique du point' AND _ECTS_='6' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Michèle' AND _PRENOM_ENS_='Hollecker' AND _EMAIL_ENS_='guillaume.havard@univ-orleans.fr');
...
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Anglais 4' AND _ECTS_='3' AND _CODE_MAT_='AG');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Michèle' AND _PRENOM_ENS_='Cimolino' AND _EMAIL_ENS_='noureddine.el\_jaouhari@univ-orleans.fr');
...
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Champs électrostatiques' AND _ECTS_='6' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Norbert' AND _PRENOM_ENS_='Garnier' AND _EMAIL_ENS_='vittoria.pierfelice@univ-orleans.fr');
INSERT IGNORE INTO livret_enseignant (_ID_ENS_, _NOM_ENS_, _PRENOM_ENS_, _EMAIL_ENS_) VALUES (@id_ens1, 'Norbert', 'Garnier', 'norbert.garnier@univ-orleans.fr');
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_='Norbert' AND _PRENOM_ENS_='Garnier' AND _EMAIL_ENS_='norbert.garnier@univ-orleans.fr');
INSERT IGNORE INTO livret_responsable_module (_ID_MOD_, _ID_ENS_, _QUALITE_) VALUES (@id_mod, @id_ens1, '1');
SET @id_ens2 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Vittoria' AND _PRENOM_ENS_='Pierfelice' AND _EMAIL_ENS_='vittoria.pierfelice@univ-orleans.fr');
...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'PY', 'Thermodynamique et statique des fluides', '5', '22', '22', '16', '0', '5', '5', 'Français_', '', '', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', '', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Thermodynamique et statique des fluides' AND _ECTS_='5' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Pascal' AND _PRENOM_ENS_='Andreazza' AND _EMAIL_ENS_='nawfal.elhage\_hassan@univ-orleans.fr');
INSERT IGNORE INTO livret_enseignant (_ID_ENS_, _NOM_ENS_, _PRENOM_ENS_, _EMAIL_ENS_) VALUES (@id_ens1, 'Pascal', 'Andreazza', 'pascal.andreazza@univ-orleans.fr');
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_='Pascal' AND _PRENOM_ENS_='Andreazza' AND _EMAIL_ENS_='pascal.andreazza@univ-orleans.fr');
INSERT IGNORE INTO livret_responsable_module (_ID_MOD_, _ID_ENS_, _QUALITE_) VALUES (@id_mod, @id_ens1, '1');
SET @id_ens2 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Nawfal' AND _PRENOM_ENS_='Elhage Hassan' AND _EMAIL_ENS_='nawfal.elhage\_hassan@univ-orleans.fr');
...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'PY', 'Ondes électromagnétiques', '6', '18', '18', '0', '0', '3', '3', 'Français_', '', '', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', '', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Ondes électromagnétiques' AND _ECTS_='3' AND _CODE_MAT_='PY');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Gerald' AND _PRENOM_ENS_='Kneller' AND _EMAIL_ENS_='nawfal.elhage\_hassan@univ-orleans.fr');
INSERT IGNORE INTO livret_enseignant (_ID_ENS_, _NOM_ENS_, _PRENOM_ENS_, _EMAIL_ENS_) VALUES (@id_ens1, 'Gerald', 'Kneller', 'gerald.kneller@univ-orleans.fr');
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_='Gerald' AND _PRENOM_ENS_='Kneller' AND _EMAIL_ENS_='gerald.kneller@univ-orleans.fr');
INSERT IGNORE INTO livret_responsable_module (_ID_MOD_, _ID_ENS_, _QUALITE_) VALUES (@id_mod, @id_ens1, '1');
SET @id_ens2 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Nawfal' AND _PRENOM_ENS_='Elhage Hassan' AND _EMAIL_ENS_='nawfal.elhage\_hassan@univ-orleans.fr');
...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'MA', 'Algèbre bilinéaire et géométrie euclidienne', '4', '24', '36', '0', '0', '6', '6', 'Français_', 'Découvrir l\'algèbre bilinéaire et ses liens avec la géométrie.', '* Dual d\'un espace vectoriel,
  ...
  * Applications : coniques et quadriques, polynômes orthogonaux.', '', '', '', '$\frac{(CC+2*CT)}{3}$', 'CT', 'Avoir suivi Algèbre 2 au semestre 3.', 'Ressources', 'Biblio', '', 'Unité obligatoire.');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Algèbre bilinéaire et géométrie euclidienne' AND _ECTS_='6' AND _CODE_MAT_='MA');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Jean' AND _PRENOM_ENS_='Renault' AND _EMAIL_ENS_='noureddine.el\_jaouhari@univ-orleans.fr');
INSERT IGNORE INTO livret_enseignant (_ID_ENS_, _NOM_ENS_, _PRENOM_ENS_, _EMAIL_ENS_) VALUES (@id_ens1, 'Jean', 'Renault', 'jean.renault@univ-orleans.fr');
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_='Jean' AND _PRENOM_ENS_='Renault' AND _EMAIL_ENS_='jean.renault@univ-orleans.fr');
INSERT IGNORE INTO livret_responsable_module (_ID_MOD_, _ID_ENS_, _QUALITE_) VALUES (@id_mod, @id_ens1, '1');
SET @id_ens2 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Noureddine' AND _PRENOM_ENS_='El Jaouhari' AND _EMAIL_ENS_='noureddine.el\_jaouhari@univ-orleans.fr');
