...
/ compétence. Voies de signalisation impliquées dans le développement. Guidage axonal.', '', '', '', '', '', 'Connaissances sur la communication cellulaire (récepteurs, seconds messagers) et la physiologie générale (grandes fonctions). Bases d’embryologie et de division cellulaire fortement recommandées.', '', 'Biblio', '', '');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Physiologie de la communication et de la reproduction' AND _ECTS_='7' AND _CODE_MAT_='BO');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Olivier' AND _PRENOM_ENS_='Richard' AND _EMAIL_ENS_='jean-pierre.gomez@univ-orleans.fr');
...
INSERT IGNORE INTO livret_module (_ID_MOD_ , _CODE_MAT_, _LIBELLE_MOD_, _SEMESTRE_, _NBH_C_, _NBH_TD_, _NBH_TP_, _NBH_CTD_, _ECTS_, _COEF_, _LANGUE_, _OBJECTIF_, _DESCRIPTION_, _METHODE_EVAL_, _MOD_CC_1_, _MOD_CC_2_,  _CALCUL_NF_1_, _CALCUL_NF_2_, _PREREQUIS_, _LIEN_RESSOURCE_, _BIBLIOGRAPHIE_, _NOTE_ELIM_, _OBLIGATOIRE_) VALUES (@id_mod, 'CH', 'Stage en chimie organique', '6', '0', '0', '0', '0', '2', '2', 'Français_', '', 'Stage pratique en chimie organique.', '', '', '', '', '', 'En complément de l’UE SOL6CH10.', '', 'Biblio', '', '');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Stage en chimie organique' AND _ECTS_='2' AND _CODE_MAT_='CH');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Arnaud' AND _PRENOM_ENS_='Tatibouet' AND _EMAIL_ENS_='');
...
enzymes et/ou de leurs substrats. Relations structure-activités.', '', '', '', '', '', 'Techniques d’analyses spectroscopiques des biomolécules, Enzymologie fondamentale, Biochimie générale.', '', 'Biblio', '', '');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Biochimie moléculaire' AND _ECTS_='7' AND _CODE_MAT_='BH');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'F.Brulé' AND _PRENOM_ENS_='C.Pichon R.Daniellou' AND _EMAIL_ENS_='');
...
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Dynamique et différenciation cellulaire' AND _ECTS_='4' AND _CODE_MAT_='BO');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Martine' AND _PRENOM_ENS_='Guérin' AND _EMAIL_ENS_='jacques.pichon@univ-orleans.fr');
...
appliquées à l’étude des fonctions physiologiques et cérébrales chez l’animal. Ce module principalement pratique abordera entre autres la culture de cellules nerveuses et leur utilisation, la technique d’immunohistochimie cérébrale ainsi que les méthodes d’exploration comportementale chez le rongeur. Des techniques de physiologie seront également abordées via par exemple l’étude pharmacologique de l’automatisme cardiaque et de la pression artérielle, ...', '', '', '', '', '', 'Connaissances de physiologie animale (niveau L2)', '', 'Biblio', '', '');
SET @id_mod := (SELECT DISTINCT _ID_MOD_ FROM livret_module WHERE _LIBELLE_MOD_='Neurosciences et physiologie expérimentales' AND _ECTS_='4' AND _CODE_MAT_='BO');
INSERT IGNORE INTO livret_programme (_ID_PROMO_, _ID_MOD_) VALUES (@id_promo, @id_mod);
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_ = 'Céline' AND _PRENOM_ENS_='Montécot-Dubourg' AND _EMAIL_ENS_='stephane.mortaud@univ-orleans.fr');
INSERT IGNORE INTO livret_enseignant (_ID_ENS_, _NOM_ENS_, _PRENOM_ENS_, _EMAIL_ENS_) VALUES (@id_ens1, 'Céline', 'Montécot-Dubourg', 'celine.montecot-dubourg@univ-orleans.fr');
SET @id_ens1 := (SELECT DISTINCT _ID_ENS_ FROM livret_enseignant WHERE _NOM_ENS_='Céline' AND _PRENOM_ENS_='Montécot-Dubourg' AND _EMAIL_ENS_='celine.montecot-dubourg@univ-orleans.fr');
INSERT IGNORE INTO livret_responsable_module (_ID_MOD_, _ID_ENS_, _QUALITE_) VALUES (@id_mod, @id_ens1, '1');
